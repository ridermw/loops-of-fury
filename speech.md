...
Here is the working definition for the rest of the hour, taken almost word for word from the paper. The word “loop” is easy to misread as idle spinning, so be precise: each turn does something concrete — it finds work worth doing, hands it to an agent, verifies whether the result is right, saves state, then decides the next step. Those are the five moves — discovery, handoff, verification, persistence, and scheduling. And here is the test that makes it precise: drop any one of the five and, in the paper’s exact words, the loop will not turn, or will turn in place.

Read the five as a single sentence and they tell a story: discovery finds the work, handoff gives it to an agent, verification decides whether the result is actually right, persistence saves what happened, and scheduling wakes the whole thing up again tomorrow. The paper draws them as a cycle — scheduling closes the loop by feeding an unfinished turn into the next day’s run, and verification is the one move that can say no.

Why insist on all five? Because every broken loop in the wild is missing one of them, and the paper is blunt that a loop missing a move turns in place. A loop with no verification is just an agent nodding at itself. A loop with no persistence forgets everything by morning. A loop with no scheduling is just one run you did once, not a loop at all — as Osmani puts it, automations are what make a loop an actual loop and not just one run you did once.

So treat this slide as your checklist for the rest of the talk. Whenever I put a real loop on the screen, point at all five moves in turn. If you cannot find one of them, you have found the bug. And the very next slide walks these five in full, mapped onto one real morning-triage loop, in the paper’s own words.

---

## Slide 8 — The five moves of one loop

This is Figure 2 from the paper — the five moves of one turn, drawn as a cycle, because scheduling closes the loop: it feeds an unfinished turn into the next day’s run, and verification is the move that can say no. The paper grounds all of this in one concrete example, and I want to read it almost verbatim because it is the clearest thing in the whole piece. Osmani built himself a morning triage loop. In the morning an automation kicks off on its own. A triage skill reads yesterday’s failing CI tests, the still-open issues, and recent commits, and writes its results into a markdown file or a Linear board. For each finding worth acting on it opens an isolated worktree; one sub-agent drafts the fix, a second reviews it against the project’s skills and tests. A connector opens the pull request and updates the ticket. Anything it cannot handle goes to an inbox to wait for a human, and a state file survives so the next day picks up where this one left off.

Now walk the five, in the paper’s own words. Discovery figures out what this turn should do — in the example, the triage skill reads CI failures, open issues, and recent commits. The key is letting the agent find its own work rather than being handed a list, and crucially the automation triggers a skill — knowledge made permanent — rather than a wall of instructions pasted into a cron job nobody will update. Discovery sets the ceiling on the whole loop’s quality: surface work of no value and the other four moves are done beautifully in service of nothing. Then handoff moves the task from the scheduling system into the hands of the agent that does the work — each finding gets its own isolated git worktree, so multiple agents change code in separate directories without stepping on each other.

Verification is the easiest move to cut corners on and the one least affordable to skip. After the first sub-agent drafts the fix, a second sub-agent reviews it — different instructions, sometimes a different model. The agent that wrote the code grades its own homework too softly; a dedicated hole-picker catches what the first talked itself into letting through. This is the thing that can say no. A loop without a real check, in the paper’s exact words, is just an agent nodding at itself.

The last two close the cycle. Persistence lands the result somewhere that survives the conversation: a PR and updated ticket via a connector, an inbox for what cannot be handled, and a state file recording progress. A loop’s memory cannot live only in the context window; what is written to markdown or a board does not forget. And scheduling is what makes one turn into a loop — the triage runs automatically each morning, and the state file lets unfinished findings carry to the next day, which picks up on its own. As Osmani puts it, automations are what make a loop an actual loop and not just one run you did once.

If you forget the prose, keep the paper’s Table II in your head: discovery finds this turn’s work on its own; handoff hands the task off, isolated; verification swaps in another agent to say no; persistence writes state outside the conversation; scheduling makes it turn round after round. Five moves, one turn that feeds the next. Drop any one and the loop will not turn, or will turn in place — and on the next few slides, every failure mode I show you is exactly one of these five being skipped.
